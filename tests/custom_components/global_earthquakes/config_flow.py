    ...
    async def async_step_user(self, user_input=None) -> FlowResult:
        ...
        data_schema = vol.Schema({
            ...
            vol.Required(
                ...
                selector.NumberSelectorConfig(min=0, max=50, step=1, mode=selector.NumberSelectorMode.SLIDER)
            ),
            vol.Optional(CONF_SCAN_INTERVAL, default=DEFAULT_SCAN_INTERVAL): int,
        })
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    async def async_step_init(self, user_input=None) -> FlowResult:
        ...
        options_schema = vol.Schema({
            vol.Required(
                CONF_MIN_MAG_USA, 
                default=self.config_entry.options.get(CONF_MIN_MAG_USA, DEFAULT_MIN_MAG_USA)
            ): vol.Coerce(float),
            vol.Required(
                CONF_MIN_MAG_GLOBAL, 
                default=self.config_entry.options.get(CONF_MIN_MAG_GLOBAL, DEFAULT_MIN_MAG_GLOBAL)
            ): vol.Coerce(float),
            vol.Required(
                CONF_MONITORED_COUNTRIES, 
                default=self.config_entry.options.get(CONF_MONITORED_COUNTRIES, ["ALL"])
            ): selector.SelectSelector(
                selector.SelectSelectorConfig(
                    options=COUNTRY_OPTIONS,
                    multiple=True,
            ...
            vol.Required(
                CONF_MAP_MARKERS, 
                default=self.config_entry.options.get(CONF_MAP_MARKERS, DEFAULT_MAP_MARKERS)
            ): selector.NumberSelector(
                selector.NumberSelectorConfig(min=0, max=50, step=1, mode=selector.NumberSelectorMode.SLIDER)
            ),
            vol.Optional(
