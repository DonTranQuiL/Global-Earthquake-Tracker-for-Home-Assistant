...
async def test_coordinator_and_dynamic_sensors(hass: HomeAssistant, aioclient_mock):
    ...
    mock_usgs_payload = {
        "features": [
            {
                "id": "eq_japan",
                "properties": {"mag": 6.1, "place": "Tokyo, Japan", "time": 1700000000000, "type": "earthquake", "tsunami": 1, "alert": "yellow"},
                "geometry": {"coordinates": [139.0, 35.0, 10.0]}
            },
            {
                "id": "eq_cali_large",
                "properties": {"mag": 3.5, "place": "Los Angeles, CA", "time": 1700000000000, "type": "earthquake", "tsunami": 0},
                "geometry": {"coordinates": [-118.0, 34.0, 5.0]}
            },
            {
                "id": "eq_cali_small", # Should be filtered out because it is < 2.5 in the USA
                "properties": {"mag": 1.5, "place": "San Francisco, CA", "time": 1700000000000, "type": "earthquake", "tsunami": 0},
    ...
    with patch("custom_components.global_earthquakes.cache.GlobalEarthquakeCache.load_cache", return_value=[]), \
         patch("custom_components.global_earthquakes.cache.GlobalEarthquakeCache.save_cache"):
         
        assert await hass.config_entries.async_setup(entry.entry_id)
        await hass.async_block_till_done()

    # 1. Verify the Static Main Sensor
    main_sensor = hass.states.get("sensor.earthquake_tracker_global_test_latest_magnitude")
    ...
    japan_sensor = hass.states.get("sensor.earthquake_tracker_global_test_6_1m_tokyo_japan")
    ...
    assert japan_sensor.attributes.get("entity_picture") == "/global_earthquakes_assets/tsunami.png"

    # Verify the larger California quake spawned
    cali_sensor = hass.states.get("sensor.earthquake_tracker_global_test_3_5m_los_angeles_ca")
